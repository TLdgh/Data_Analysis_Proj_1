...
# Conclusion
We want to find some interesting conclusions in the data of US presidential election poll data in 2016. To achieve the aim, we use pie charts, bar charts, map charts, box charts, scatter charts, and line charts to visualize the data and find the relationship between different variables.  

First, we give a variables dictionary which helps readers to understand data easily and clearly. When analyzing the relationship between pollsters and grades, we use a pie chart. Pie charts are effective for showing the composition of whole grades in terms of their parts. After that, we try to find out the relationship between sample size and grade. Bar charts are excellent for comparing the sample sizes of different grades. In addition, bar charts can be easily customized to show data in different ways, such as grouped bars, stacked bars, or horizontal bars. Therefore, we use the stacked bar chart to show the votes of 4 candidates in each state. Since the votes of the other 2 candidates are so small, we can ignore them in subsequent analysis. Then Using the line charts, we create the lines of cumulative mean for the 2 candidates. This chart is excellent for showing trends and changes over time, making it suitable for time series data. Finally, we create map charts to show Clinton’s and Trump’s vote share in each state, and we also use map charts to compare the predicted results and final results. Because map charts allow for easy comparison of data across different geographical areas. From this map chart, we can find the results of 6 states are different from the prediction.
# References