...
```{r, warning=FALSE, out.height="30%", fig.cap = "Raw Poll Proportion by Time"}
...
\end{minipage}
Furthermore, Clinton is clearly ahead of Trump in states like California, DC and Hawaii etc. On the nother side, Trump is ahead of Clinton in Alabama, Alaska and Idaho etc. This helps us explore the leading candidates in each state.
...
shots<-webshot(url = "./Figures/TsChart.html", file = "./Figures/TsChart.png", delay = 1, zoom = 1, vheight = 300)
```

![Adjusted Poll Percentage by Time](./Figures/TsChart.png){height=50%}
