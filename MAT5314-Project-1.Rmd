...
```{r}
head(ElectionPoll[which(is.na(ElectionPoll$grade)==TRUE), ], 3)
```
...
### Sample Size
Checking the sample size alone does not inform us much about the poll result. Instead, we take a look at the distribution of the sample size with regards to the grade:

```{r, warning=FALSE, fig.cap = "Distribution of Sample Size for Grades", fig.width=4, fig.height=4}
...
### Population
We now take a look at the population type for which the poll was sampled.
...
popChart <- plot_ly(GradeVsPop, x = ~grade, y = ~population, 
               ...
               size = ~Size, color = ~Size, colors = 'Blues',
               marker = list(symbol="circle", sizemode = 'diameter', line = list(width = 2, color = 'black')))%>%
  layout(title = 'Population Type VS Grade',
...
shots<-webshot(url = "./Figures/popChart.html", file = "./Figures/popChart.png", delay = 1, zoom = 4)
```

![Population Type with Size for Grades](./Figures/popChart.png){width=50%}
...
```{r, warning=FALSE, fig.cap = "Raw Poll Proportion by Time.", fig.width=6, fig.height=4, fig.pos="H"}
...
```{r, fig.cap = "Poll Percentage by States.", fig.width=6, fig.height=4, fig.pos="H"}
...
```{r, fig.cap = "Adjusted Poll Percentage by States", fig.width=6, fig.height=4}
#Create frame contained time, rawpoll
...
![Boxchart of Difference between Polls](./Figures/boxchart.png){width=70%}
...
shots<-webshot(url = "./Figures/mapchart_1_2.html", file = "./Figures/mapchart_1_2.png", delay = 2, zoom = 2, vheight = 400, vwidth = 500)
```
...
From the above plots we can see that the actual final election outcome was almost the entire opposite to the predicted result.
