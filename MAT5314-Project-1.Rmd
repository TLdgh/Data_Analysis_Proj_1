...
saveWidget(widget = piechart, file = "piechart.html")
webshot(url = "piechart.html", file = "piechart.png", delay = 1, zoom = 4, vheight = 500)
```
...
```{r, warning=FALSE, fig.cap = "Raw Poll Proportion by Time.", fig.pos="H"}
...
```{r, fig.cap = "Poll Percentage by States.", fig.pos="H"}
...
Furthermore, based on the data given and previous analysis, we conclude that Clinton and Trump will compete to become the new President of the United States. We're eager to find out exactly the states which Clinton or Trump captured all of the electoral votes. The visualization below shows the net polls generated by each state for Clinton and Trump. 
...
# Calculate net polls
poll_gap <- data.frame(
  state = clinton_state$state,
  gap = clinton_state$ClintonPolls - trump_state$TrumpPolls
)
```


```{r, fig.cap = "Net Polls by States.", fig.pos="H"}
...
It is not difficult to see from Figure 3 that Maine and Nebraska have two and three additional electoral district respectively in addition to their own states. The candidate with the most votes in Maine and Nebraska will receive two electoral votes, and the remaining electoral votes will be allocated to the presidential candidate who wins each district. Moreover, DC have three electoral votes as an independent district. Based on RawPoll data, the chart above shows Clinton won 27 constituencies including 25 states, one additional Maine district and DC. Trump won 25 states plus one additional district in Maine and three additional districts in Nebraska.
...
saveWidget(widget = boxchart, file = "boxchart.html")
webshot(url = "boxchart.html", file = "boxchart.png", delay = 1, zoom = 4, vheight = 500)
```
...
#Clean the data
ElectionPoll[which(ElectionPoll$state=="Maine CD-1"),"state"]<-"Maine"
ElectionPoll[which(ElectionPoll$state=="Maine CD-2"),"state"]<-"Maine"
ElectionPoll[which(ElectionPoll$state=="Nebraska CD-1"),"state"]<-"Nebraska"
ElectionPoll[which(ElectionPoll$state=="Nebraska CD-2"),"state"]<-"Nebraska"
ElectionPoll[which(ElectionPoll$state=="Nebraska CD-3"),"state"]<-"Nebraska"
#apply rMean to all states
Navg_lst<-ElectionPoll%>%select(state, enddate, rawpoll_trump, rawpoll_clinton)%>%arrange(.by=enddate, decreasing=FALSE)%>%split(.$state)
...
saveWidget(widget = cMeanchart, file = "cMeanchart.html") 
webshot(url = "cMeanchart.html", file = "cMeanchart.png", delay = 1, zoom = 4, vheight = 600)
```
...
saveWidget(widget = mapchart, file = "mapchart.html") 
webshot(url = "mapchart.html", file = "mapchart.png", delay = 3, zoom = 2, vheight = 600)
```
...
saveWidget(widget = mapchart3, file = "mapchart3.html") 
webshot(url = "mapchart3.html", file = "mapchart3.png", delay = 3, zoom = 2, vheight = 600)
```
