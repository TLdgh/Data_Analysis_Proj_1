...
We immediately noticed that some of the poll rate fell below zero. 
...
df_4<-merge(df_3,electionresult,by.x="State")
bin<-matrix(0,nrow(df_4),1)
df_4<-cbind(df_4,bin)
...
mapchart3<-plot_geo(df_4, locationmode = 'USA-states')%>% 
  add_trace(z = ~bin, locations = ~Code, color =~bin, colors=c('indianred','white','skyblue'),
           colorbar=list(limits = c(0, 100),title = "cliton win in this state",x = 1, y = 0.9))%>%
  add_trace(type = "scattergeo", locationmode = 'USA-states', locations = ~Code, 
            text = ~Number, mode = "text",textfont = list(size = 10))%>%
  layout(geo = g)%>%
  layout(title = '2016 Presidential Election Results')
htmlwidgets::saveWidget(widget = mapchart3, file = "mapchart3.html") 
webshot::webshot(url = "mapchart3.html", file = "mapchart3.png", delay = 1, zoom = 4, vheight = 600)
```
