...
To make state-by-state polling results more visible, we've reflected the candidates' polling shares on a map of the United States. Since the other candidates are polling far behind Clinton and Trump, we select the Clinton and Trump polls and ignore the other candidates' polls. In the choropleth plots below, blue indicates that Clinton is polling proportionally greater than Trump, and red indicates that Trump is polling proportionally greater than Clinton. The darker the color is, the more overwhelming the proportions are. For states that are nearly white, the margin of victory is nearly half between Trump and Clinton.
```{r}
#fill a matrix
ElectionPoll<-read.csv("polls_us_election_2016.csv", header = TRUE)
...
htmlwidgets::saveWidget(widget = mapchart1, file = "mapchart1.html")
...
htmlwidgets::saveWidget(widget = mapchart2, file = "mapchart2.html") 
