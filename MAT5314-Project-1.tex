...
\cellcolor{gray!6}{enddate} & \cellcolor{gray!6}{4208} & \cellcolor{gray!6}{character} & \cellcolor{gray!6}{2016-11-06, 2016-11-07, 2016-11-05} & \cellcolor{gray!6}{345} & \cellcolor{gray!6}{0} & \cellcolor{gray!6}{End date of poll}\\
...
We supposed that there were two possible reasons for these missing data:
one is that there were some errors of data in the original file; the
...
data from other variables would be omitted as well, which would affect
the final result and may yield incorrect inference to the data.
Therefore, we only deleted a variable entirely if it had most of its
values missing. Dividing the variable (column) will not affect values in
other variables like omitting NA by rows.
...
replaced ``NA'' with the actual grades and ``none''. The result was
...
Because some pollsters were repeated in different rows of the data set,
so we wanted to verify that each pollster corresponds to only one kind
of grade. The result was presented as follows:
...
the unrated pollsters maked up the largest percentage, about 15.8\% of
the total; pollsters with grades ``B'' and ``C+'' accounted for the
second and third most, 12.8\% and 12.2\% respectively; ``D'' grade had
the smallest percentage of pollsters, only about 0.51\%. Besides,
B-level grades, including ``B+'', ``B'' and ``B-'', were around 34.7\%,
almost one-third of the total; C-level and A-level grades were about
21.89\% and 14.79\% respectively. For those without explicit grades,
whose grades were ``A/B'', ``B/C'' and ``C/D'', they accounted only for
12.24\%, ``B'' grade pollsters maked up the majority of this part
especially, almost 9.69\%.
...
{\centering \includegraphics{./Figures/piechart} 

}

\caption{Pollster's Grade Distribution}\label{fig:unnamed-chunk-9}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{MAT5314-Project-1_files/figure-latex/unnamed-chunk-10-1} \caption{Distribution of Sample Size for Grades}\label{fig:unnamed-chunk-10}
\end{figure}
\end{minipage}
...
\begin{figure}
\includegraphics[width=1\linewidth,height=0.7\textheight]{MAT5314-Project-1_files/figure-latex/unnamed-chunk-17-1} \caption{Poll Percentage by States}\label{fig:unnamed-chunk-17}
\end{figure}
\end{minipage}
...
candidates' polling shares on a map of the United States.
...
{\centering \includegraphics{./Figures/mapchart_1_2} 

}

\caption{Map Chart of Vote Shares of Candidates by States}\label{fig:unnamed-chunk-26}
\end{figure}
...
In the contour plot Figure 9 on the left, blue indicated that Clinton
was polling proportionally greater than Trump, and red indicated that
Trump was polling proportionally greater than Clinton. The darker the
color, the higher the proportions were. For states that were nearly
white, the margin of victory was nearly half between Trump and Clinton.

\vspace{13pt}

We again saw that there was little difference between the results using
the raw poll data and the adjusted poll data, which further supported
our opinion that the adjustment made by FiveThirtyEight must be
carefully justified by readers because of its negative values. We then
saw from the map in Figure 9 that Clinton seemed to lead the public
opinions a little bit more because the colors of states were closer to
white and pale red.
...
Furthermore, we wanted to find out exactly the states which Clinton or
Trump captured all of the electoral votes. The visualization below
showed the net polls generated by each state for Clinton and Trump.

\begin{minipage}[t]{0.7\textwidth}
\begin{figure}
\includegraphics[width=1\linewidth]{MAT5314-Project-1_files/figure-latex/unnamed-chunk-27-1} \caption{Net Polls by States.}\label{fig:unnamed-chunk-27}
\end{figure}
\end{minipage}
\begin{minipage}[t]{0.3\textwidth}
\vspace{0pt}
...
and the remaining electoral votes was allocated to the presidential
candidate who won each district. Moreover, DC have three electoral votes
as an independent district.

Finally, we took a look at the map chart of the electoral votes based on
raw poll result. We used the poll opinion to generate a map showing the
predicted election result, and we compared it with the counterpart
...
{\centering \includegraphics{./Figures/mapchart_3_4} 

}

\caption{Map Chart of Election Result 2016}\label{fig:unnamed-chunk-29}
\end{figure}
...
\begin{col}{0.38\textwidth}
From the Figure 11 we could see that the actual final election outcome
was opposite to the predicted result, since in the final election Trump
won five states with influential number of votes. In 2016, Trump won 304
votes compared to 227 votes for Hillary Clinton, which was not predicted
by the election poll result. Therefore, readers must be careful about
the election poll. There have been some criticisms on the usefulness of
these polls (\protect\hyperlink{ref-Critic2}{2022b}). From the previous
analysis in Figure 1, we were also able to find out that over half of
the pollster ratings were between B+ to C and none. This might suggest
that public opinion polls are not as reliable as one would expect due to
the quality of the pollsters.
